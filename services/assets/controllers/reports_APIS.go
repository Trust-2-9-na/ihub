...
func (c *Construct) UpdateWeeklyReport(w http.ResponseWriter, r *http.Request) {
	...
	// 9️⃣ Fetch updated report with graceful fallback
	var reloadedReport models.WeeklyReport
	reloadErr := c.DB.Preload("ProgressItems").Preload("Comments.User.Profile").Preload("Student").First(&reloadedReport, report.ID).Error

	if reloadErr != nil {
		// Try simpler reloads as fallback
		if err := c.DB.Preload("ProgressItems").Preload("Comments").First(&reloadedReport, report.ID).Error; err != nil {
			if err := c.DB.First(&reloadedReport, report.ID).Error; err != nil {
				// If all reloads fail, return the saved report (without full associations) with a warning
				c.Json(w, http.StatusOK, "Weekly report updated successfully", map[string]interface{}{
					"data":    report,
					"warning": "Report was updated but could not be reloaded with full associations. Please refresh to see complete data.",
				})
				return
			}
		}
